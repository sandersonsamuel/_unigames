...
export async function listGames(isCompetition?: boolean) {
  const baseQuery = db
    .select({
      id: schema.games.id,
  ...
  const query = isCompetition !== undefined
    ? baseQuery.where(eq(schema.games.competition, isCompetition))
    : baseQuery;

  return await query.orderBy(desc(schema.games.id));
}
